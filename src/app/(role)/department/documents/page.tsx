...
const MOCK_DOCUMENTS: Document[] = [
  ...
  },
  {
    ...
    description: "Strategic product development roadmap for the next year",
    fileType: "pptx",
    size: 4120000,
  ...
  },
  {
    ...
    description: "Analysis of the quarterly customer satisfaction survey",
    fileType: "xlsx",
    size: 1560000,
...
// Helpers
// Will be used for formatting file sizes in the future
...
export default function DepartmentDocumentsPage() {
        ...
        setTimeout(() => {
          ...
          if (activeTab !== "all") {
            if (activeTab === "owned") {
              ...
              // Mock shared documents (in production this would come from API)
              filteredDocs = filteredDocs.filter((doc) =>
                ["1", "3", "5"].includes(doc.id)
              );
      ...
      setDocuments((prev) => [
        {
          ...
          department: "Information Technology", // Mock current user department
          owner: {
            id: "101",
            name: "John Smith",
  ...
  const columns = [
        ...
        <div className="flex items-start">
          ...
          <div>
            <Text strong className="text-gray-900 hover:text-primary-600">
              <Link href={`/department/documents/${record.id}`}>{record.title}</Link>
            </Text>
            {record.description && (
    ...
    },
    {
      ...
      key: "status",
      width: 150,
      render: (status: "draft" | "pending_approval" | "approved" | "rejected" | "published", record: Document) => (
        <Select
          value={status}
          onChange={(newStatus: "draft" | "pending_approval" | "approved" | "rejected" | "published") => {
            // Quick status update
            setDocuments((prev) =>
              prev.map((doc) =>
                doc.id === record.id
        ...
        >
          ...
          <Option value="published">
            <Badge status="processing" text="Published" />
          </Option>
        </Select>
      ),
    },
          ...
          overlay={
            <Menu>
              <Menu.Item key="view" icon={<EyeOutlined />}>
                <Link href={`/department/documents/${record.id}`}>View</Link>
              </Menu.Item>
              <Menu.Item 
                key="edit" 
                icon={<EditOutlined />}
        ...
        <Form form={uploadForm} layout="vertical" onFinish={handleUpload}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              ...
              {/* Admin specific: Status selection */}
              <Form.Item
                name="status"
