...
export default function DepartmentDocumentsList() {
  ...
  const fetchDocuments = useCallback(async () => {
    setLoading(true);
    try {
      const { data } = await DepartmentApi.getDocuments(filters);
  ...
  const handleEditSave = async (values: any) => {
    try {
      await DepartmentApi.updateDocument(editingDocument!.id, values);
  ...
  const handleDelete = async (id: string) => {
    modal.confirm({
      ...
      onOk: async () => {
        try {
          await DepartmentApi.deleteDocument(id);
      ...
      render: (text: string, record: Document) => (
        <div>
          <div className="font-medium text-blue-600">
            <Link href={`/department/documents/${record.id}`}>{text}</Link>
          </div>
          ...
          overlay={
            <Menu>
              <Menu.Item key="view" icon={<EyeOutlined />}>
                <Link href={`/department/documents/${record.id}`}>View Details</Link>
              </Menu.Item>
